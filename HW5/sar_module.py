...
'''
import os
os.chdir('/Users/user/Desktop/ITMO/Scientific_Python/ITMO_ScientificPython_2024/HW5/')
...
def desc_calc(data, mode = 'train', log=None) -> pd.DataFrame: # fill the necessary arguments
    return getAllDescriptors(data, mode, log)# compute all descriptors
...
def sar_model_evaluation(descriptors: pd.DataFrame):
    ...
    model1, y_pred1, metrics1 = fit_Ridge(X_train, X_test, y_train, y_test) # hyperparameters tuning with selected function
...
def sar_model_train(descriptors_train: pd.DataFrame, indices): 
    ...
    b = StandardScaler() 
    clf = RidgeCV(cv = 10) # selected model from sklearn
...
if __name__ == "__main__":
    ...
    # loading data
    train_data = pd.read_csv('/Users/user\Desktop/ITMO/Scientific_Python/ITMO_ScientificPython_2024/HW5/logp_100.csv')
    pred_data = pd.read_csv('/Users/user\Desktop/ITMO/Scientific_Python/ITMO_ScientificPython_2024/HW5/logp_inputs.csv')
    cpds = [row for row in pred_data.loc[:, 'SMILES']]

    # calculating descriptors
    print("Calculating descriptors for training data...")
    train_descriptors = desc_calc(train_data, mode = 'train', log = None)
    print("Calculating descriptors for prediction data...")
    pred_descriptors = desc_calc(pred_data, mode = '', log = None)
    ...
    final_model = sar_model_train(descriptors_train = train_descriptors, indices = cols)
